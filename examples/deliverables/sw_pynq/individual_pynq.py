#!/usr/bin/env python3
"""
PYNQ host example for the final-project `individual` design.
...
Typical use:
  ./individual_pynq.py
  ./individual_pynq.py --dut xor
  ./individual_pynq.py --bitstream /path/to/design_1_wrapper.bit
  ./individual_pynq.py --bitstream /path/to/design_1_wrapper.bit --hwh /path/to/design_1.hwh
  ./individual_pynq.py /path/to/design_1_wrapper.bit
  ./individual_pynq.py --no-program
"""
...
def compute_expected(dut: str, a: int, b: int) -> tuple[int, int]:
    if dut == "xor":
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the individual AXI-stream DUT through PYNQ.",
    ...
    parser.add_argument(
        ...
        choices=("mul", "xor"),
        default="mul",
        help="Expected DUT behavior used for validation and default bitstream lookup",
    )
...
def stage_overlay_files(bitstream: Path, hwh: Path) -> tuple[tempfile.TemporaryDirectory[str] | None, Path]:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="individual_pynq_")
    ...
    try:
        ...
        try:
            ...
            for index, (a, b) in enumerate(TESTS):
                expected_lo, expected_hi = compute_expected(args.dut, a, b)
                ...
                actual_lo = int(output_buf[0])
                actual_hi = int(output_buf[1])
