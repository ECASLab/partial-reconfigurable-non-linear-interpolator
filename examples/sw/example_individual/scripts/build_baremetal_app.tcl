...
if {$argc < 6} {
    error "Usage: build_baremetal_app.tcl <action> <dut> <part> <xsa> <workspace> <app_src>"
}

set action    [string tolower [lindex $argv 0]]
set dut_impl  [string tolower [lindex $argv 1]]
...
set platform_name "dut64_platform"
set domain_name   "standalone_domain"
set app_name      "dut64_dma_smoke"
...
if {$action eq "platform"} {
    puts "Creating standalone platform in $workspace"
...
if {$action eq "app"} {
    puts "Creating empty application in $workspace"
    ...
    if {$dut_impl eq "xor"} {
        app config -name $app_name define-compiler-symbols APP_DUT_IMPL_XOR
    } else {
        app config -name $app_name define-compiler-symbols APP_DUT_IMPL_MUL
    }
    exit
}

puts "Building application in $workspace"
